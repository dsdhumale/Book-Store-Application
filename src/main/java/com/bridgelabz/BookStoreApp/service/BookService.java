...
public class BookService implements IBookService{
    ...
    public BookModel deleteBookById(String token, long bookId) throws BookStoreException {
        long id = tokenUtil.decodeToken(token);
        UserModel user = userRepo.findById(id).orElseThrow(() -> new BookStoreException("Token do not match the user"));
        BookModel bookModel =bookRepo.findById(bookId).orElseThrow(() -> new BookStoreException("Books id " + bookId + " not found!"));
        if (user!= null) {
            bookRepo.deleteById(bookId);
            return bookModel;
        } else throw new BookStoreException("User dont have authorization");
    }
     ...
     public BookModel updateQuantity(String token, long bookId, int quantity) throws BookStoreException {
        long id = tokenUtil.decodeToken(token);
        UserModel user = userRepo.findById(id).orElseThrow(() -> new BookStoreException("Token do not match the user"));
        BookModel book =bookRepo.findById(bookId).orElseThrow(() -> new BookStoreException("Books id " + bookId + " not found!"));        
